...
export function ContactOwnerCard({ property }: { property: Property }) {
  ...
  const handleShare = async () => {
    ...
    if (navigator.share) {
      await navigator.share({ title: property.title, text: `Check out this property on Gojo Homes: ${property.title}`, url });
    } else {
        ...
        <div style={{ background: '#f0f6ff', border: '1px solid #dbeafe', borderRadius: 12, padding: '16px', marginBottom: 16 }}>
          ...
          <div style={{ display: 'grid', gap: 8 }}>
            ...
            <div style={{ fontSize: 12, color: '#6b7280', textAlign: 'center' as const }}>
              Or{' '}
              <button onClick={() => router.push('/sign-in')} style={{ background: 'none', border: 'none', color: '#006AFF', fontSize: 12, fontWeight: 600, cursor: 'pointer', padding: 0 }}>
                sign in
              </button>{' '}
              for full messaging
            </div>
