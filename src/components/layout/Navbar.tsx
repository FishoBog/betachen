...
import { Home, PlusCircle, MessageSquare, LayoutDashboard, Map, Heart, Bell, GitCompare, Settings, Menu, X } from 'lucide-react';
    ...
    return (
      <Link href={href}
        ...
        className={cn(
          ...
          active ? 'text-white font-semibold' : 'text-white/70 hover:text-white hover:bg-white/10'
        )}
        style={active ? { background: 'var(--coral)' } : {}}>
      ...
      <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
        <Link href="/" className="flex items-center gap-2.5 flex-shrink-0">
          <div className="w-9 h-9 rounded-xl flex items-center justify-center text-white text-lg font-bold"
            style={{ background: 'var(--coral)' }}>ጎ</div>
          <span className="font-bold text-white text-xl hidden sm:block">ጎጆ</span>
        </Link>
