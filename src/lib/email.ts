...
const FROM = process.env.RESEND_FROM_EMAIL ?? 'noreply@betachen.com';

export async function sendWelcomeEmail(to: string, name: string) {
  await resend.emails.send({
    from: FROM,
    to,
    subject: 'Welcome to ቤታችን Homes 🏠',
    html: `
      <div style="font-family:sans-serif;max-width:600px;margin:0 auto;padding:40px 24px;background:#ffffff">
        ...
        <p style="color:#4b5563;line-height:1.7">Thank you for joining ቤታችን Homes. You can now search properties, post listings, and connect with owners across Ethiopia.</p>
        <div style="margin:32px 0">
          <a href="https://www.betachen.com" style="background:#E8431A;color:white;padding:14px 28px;border-radius:10px;text-decoration:none;font-weight:700;font-size:16px">Browse Properties 🏘️</a>
        </div>
...
export async function sendListingApprovedEmail(to: string, listingTitle: string, listingId: string) {
  await resend.emails.send({
    from: FROM,
    to,
    subject: '✅ Your listing is now LIVE on ቤታችን Homes!',
    html: `
      <div style="font-family:sans-serif;max-width:600px;margin:0 auto;padding:40px 24px;background:#ffffff">
        <div style="text-align:center;margin-bottom:32px">
          <h1 style="font-size:32px;font-weight:900;color:#006AFF;margin:0">ቤታችን Homes</h1>
        ...
        <p style="color:#4b5563;line-height:1.7">Your property listing <strong>${listingTitle}</strong> has been reviewed and approved. It is now live on ቤታችን Homes and visible to thousands of buyers and renters.</p>
        <div style="margin:32px 0">
          <a href="https://www.betachen.com/properties/${listingId}" style="background:#E8431A;color:white;padding:14px 28px;border-radius:10px;text-decoration:none;font-weight:700;font-size:16px">View Your Listing 🏠</a>
        </div>
  ...
  await resend.emails.send({
    ...
    html: `
      <div style="font-family:sans-serif;max-width:600px;margin:0 auto;padding:40px 24px;background:#ffffff">
        ...
        <div style="margin:32px 0">
          <a href="https://www.betachen.com/owner/listings/${listingId}/renew" style="background:#E8431A;color:white;padding:14px 28px;border-radius:10px;text-decoration:none;font-weight:700;font-size:16px">Renew for ETB 300 🔄</a>
        </div>
        <hr style="border:none;border-top:1px solid #e5e7eb;margin:32px 0"/>
        <p style="color:#9ca3af;font-size:13px;text-align:center">ቤታችን Homes · betachen.com · Addis Ababa, Ethiopia 🇪🇹</p>
      </div>
...
export async function sendReservationConfirmedEmail(to: string, propertyTitle: string, checkIn: string, checkOut: string, amount: number) {
  await resend.emails.send({
    from: FROM,
    to,
    subject: '🎉 Reservation Confirmed — ቤታችን Homes',
    html: `
      <div style="font-family:sans-serif;max-width:600px;margin:0 auto;padding:40px 24px;background:#ffffff">
        <div style="text-align:center;margin-bottom:32px">
          <h1 style="font-size:32px;font-weight:900;color:#006AFF;margin:0">ቤታችን Homes</h1>
