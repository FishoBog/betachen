...
export default function AdvertisePage() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'white' }}>
      ...
      {/* Hero */}
      <div style={{ background: 'linear-gradient(135deg, #0f172a 0%, #1e293b 60%, #0f3460 100%)', padding: '80px 24px 88px', textAlign: 'center' as const, position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'relative', maxWidth: 780, margin: '0 auto' }}>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: '#E8431A', borderRadius: 20, padding: '8px 20px', marginBottom: 24 }}>
            <Megaphone size={15} color="white" />
            <span style={{ color: 'white', fontSize: 14, fontWeight: 700, letterSpacing: '0.8px' }}>
              {lang === 'EN' ? 'ADVERTISE ON ቤታችን' : 'በቤታችን ላይ ያስተዋውቁ'}
            </span>
          </div>
          <h1 style={{ fontSize: 'clamp(34px, 5vw, 54px)', fontWeight: 900, color: 'white', lineHeight: 1.1, marginBottom: 18 }}>
            {lang === 'EN' ? 'Reach Thousands of' : 'ሺዎችን'}<br />
            <span style={{ color: '#38bdf8' }}>{lang === 'EN' ? 'Property Buyers & Owners' : 'የቤት ገዢዎችና ባለቤቶች ይድረሱ'}</span>
          </h1>
          <p style={{ color: 'rgba(255,255,255,0.8)', fontSize: 18, marginBottom: 44, lineHeight: 1.7 }}>
            {lang === 'EN'
          ...
          <div style={{ display: 'flex', justifyContent: 'center', gap: 56, flexWrap: 'wrap' as const }}>
            {[
              ...
              <div key={n} style={{ textAlign: 'center' as const }}>
                <Icon size={22} color="rgba(255,255,255,0.4)" style={{ marginBottom: 6 }} />
                <div style={{ fontSize: 30, fontWeight: 900, color: 'white' }}>{n}</div>
                <div style={{ fontSize: 13, color: 'rgba(255,255,255,0.55)', marginTop: 3, fontWeight: 600 }}>{lang === 'EN' ? en : am}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* Why advertise */}
      <div style={{ background: '#f9fafb', padding: '72px 24px', borderBottom: '1px solid #e5e7eb' }}>
        <div style={{ maxWidth: 1500, margin: '0 auto' }}>
          <h2 style={{ fontSize: 38, fontWeight: 900, color: '#111827', textAlign: 'center' as const, marginBottom: 14 }}>
            {lang === 'EN' ? 'Why Advertise on ቤታችን?' : 'ለምን በቤታችን ላይ ያስተዋውቁ?'}
          </h2>
          <p style={{ color: '#6b7280', fontSize: 18, textAlign: 'center' as const, marginBottom: 48 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 24 }}>
            {[
              ...
              <div key={en} style={{ background: 'white', borderRadius: 16, padding: '30px 26px', border: '1px solid #e5e7eb' }}>
                <div style={{ width: 56, height: 56, borderRadius: 14, background: color, display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 16 }}>
                  <Icon size={28} color={iconColor} />
                </div>
                <div style={{ fontSize: 19, fontWeight: 700, color: '#111827', marginBottom: 8 }}>{lang === 'EN' ? en : am}</div>
                <div style={{ fontSize: 15, color: '#6b7280', lineHeight: 1.6 }}>{lang === 'EN' ? descEn : descAm}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <div style={{ padding: '72px 24px', background: 'white' }}>
        <div style={{ maxWidth: 1500, margin: '0 auto' }}>
          ...
          <p style={{ color: '#6b7280', fontSize: 18, textAlign: 'center' as const, marginBottom: 40 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(240px, 1fr))', gap: 16 }}>
            {CATEGORIES.map(({ key, labelEn, labelAm, icon: Icon, color, iconColor }) => (
              <div key={key} style={{ background: color, borderRadius: 14, padding: '22px 18px', display: 'flex', alignItems: 'center', gap: 14 }}>
                <div style={{ width: 46, height: 46, borderRadius: 10, background: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                  <Icon size={22} color={iconColor} />
                </div>
                <div style={{ fontSize: 15, fontWeight: 700, color: '#111827' }}>{lang === 'EN' ? labelEn : labelAm}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* Pricing */}
      <div style={{ background: '#f9fafb', padding: '80px 24px', borderTop: '1px solid #e5e7eb' }}>
        <div style={{ maxWidth: 1500, margin: '0 auto' }}>
          <h2 style={{ fontSize: 38, fontWeight: 900, color: '#111827', textAlign: 'center' as const, marginBottom: 14 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 22 }}>
            {PACKAGES.map(pkg => (
              <div key={pkg.key}
                ...
                style={{ background: 'white', borderRadius: 18, border: `2px solid ${selectedPackage === pkg.key ? pkg.color : '#e5e7eb'}`, padding: '30px 26px', cursor: 'pointer', position: 'relative', transition: 'all 0.2s', boxShadow: pkg.popular ? '0 8px 24px rgba(0,0,0,0.1)' : '0 1px 4px rgba(0,0,0,0.06)' }}
                ...
                <div style={{ fontSize: 21, fontWeight: 900, color: pkg.color, marginBottom: 4 }}>
                  {lang === 'EN' ? pkg.labelEn : pkg.labelAm}
                </div>
                <div style={{ fontSize: 34, fontWeight: 900, color: '#111827', marginBottom: 4 }}>
                  ETB {pkg.price.toLocaleString()}
                </div>
                ...
                <div style={{ display: 'grid', gap: 8, marginBottom: 24 }}>
                  {pkg.features.map((f, i) => (
                    <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 8 }}>
                      <CheckCircle size={16} color={pkg.color} style={{ flexShrink: 0, marginTop: 1 }} />
                      <span style={{ fontSize: 14, color: '#374151', lineHeight: 1.4 }}>{lang === 'EN' ? f.en : f.am}</span>
                    </div>
                  ))}
                </div>
                <button style={{ width: '100%', padding: '13px', borderRadius: 10, background: pkg.color, color: 'white', fontWeight: 700, fontSize: 15, border: 'none', cursor: 'pointer' }}>
      ...
      <div style={{ padding: '72px 24px', background: 'white' }}>
        <div style={{ maxWidth: 1000, margin: '0 auto' }}>
          <h2 style={{ fontSize: 34, fontWeight: 900, color: '#111827', textAlign: 'center' as const, marginBottom: 40 }}>
          ...
          <div style={{ display: 'grid', gap: 16 }}>
            {[
              ...
              <div key={qEn} style={{ background: '#f9fafb', borderRadius: 14, padding: '22px 26px', border: '1px solid #e5e7eb' }}>
                <div style={{ fontSize: 17, fontWeight: 700, color: '#111827', marginBottom: 8 }}>{lang === 'EN' ? qEn : qAm}</div>
                <div style={{ fontSize: 15, color: '#6b7280', lineHeight: 1.7 }}>{lang === 'EN' ? aEn : aAm}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* Bottom CTA */}
      <div style={{ background: '#006AFF', padding: '72px 24px', textAlign: 'center' as const }}>
        ...
        <button onClick={() => setStep('form')}
          style={{ padding: '16px 42px', background: '#E8431A', color: 'white', borderRadius: 12, fontWeight: 700, fontSize: 17, border: 'none', cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: 8 }}>
