...
import { Upload, MapPin, Home, DollarSign, CheckCircle, ArrowRight, ArrowLeft, X } from 'lucide-react';
...
export default function NewListingPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const supabase = createBrowserClient();
      const locationParts = [form.specific_location, form.kebele, form.woreda, form.subcity, form.city, form.region].filter(Boolean);
    ...
    <div style={{ minHeight: '100vh', background: '#f9fafb' }}>
      ...
      <div style={{ maxWidth: 780, margin: '0 auto', padding: '40px 24px' }}>
        ...
        <div style={{ display: 'flex', gap: 6, marginBottom: 32, overflowX: 'auto' as const }}>
          {steps.map((s, i) => (
            <div key={s} style={{ flex: 1, minWidth: 80 }}>
              <div style={{ height: 4, borderRadius: 2, background: step > i + 1 ? '#006AFF' : step === i + 1 ? '#006AFF' : '#e5e7eb', marginBottom: 6, transition: 'background 0.3s' }} />
          ...
          <div>
            <div style={sectionStyle}>
              ...
              <div style={{ display: 'grid', gap: 16 }}>
                ...
                <div>
                  <label style={labelStyle}>Your WhatsApp Number (for direct inquiries)</label>
            ...
            <button onClick={() => { if (!form.title || !form.price) { setError('Please fill in title and price'); return; } setError(''); setStep(2); }}
        ...
        {step === 2 && (
          <div>
              ...
              <div style={{ display: 'grid', gap: 16 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                  ...
                  <div>
                    ...
                    <input style={inputStyle} value={form.woreda} onChange={e => set('woreda', e.target.value)}
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                  <div>
                    ...
                    <input style={inputStyle} value={form.kebele} onChange={e => set('kebele', e.target.value)}
            ...
            <div style={{ display: 'flex', gap: 12 }}>
              ...
              <button onClick={() => { if (!form.region || !form.city) { setError('Please select region and city'); return; } setError(''); setStep(3); }}
        ...
        {step === 3 && (
          <div>
            <div style={sectionStyle}>
              <div style={{ fontSize: 18, fontWeight: 800, color: '#111827', marginBottom: 24 }}>Amenities & Features</div>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(130px, 1fr))', gap: 10 }}>
                {AMENITIES.map(a => (
                  <div key={a.key} onClick={() => toggleAmenity(a.key)}
                    ...
                    <div style={{ fontSize: 13, fontWeight: 600, color: form.amenities.includes(a.key) ? '#006AFF' : '#374151' }}>{a.label}</div>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div>
            <div style={sectionStyle}>
              ...
              <label style={{ display: 'block', cursor: 'pointer' }}>
                <div style={{ border: '2px dashed #d1d5db', borderRadius: 14, padding: '40px 24px', textAlign: 'center', background: '#f9fafb', transition: 'all 0.15s' }}>
                  {uploadingPhotos ? (
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(140px, 1fr))', gap: 12, marginTop: 20 }}>
                  {photoUrls.map((url, i) => (
                    <div key={url} style={{ position: 'relative', borderRadius: 10, overflow: 'hidden', aspectRatio: '4/3' }}>
                      ...
                      {i === 0 && (
                        <div style={{ position: 'absolute', top: 6, left: 6, background: '#006AFF', color: 'white', fontSize: 10, fontWeight: 700, padding: '2px 8px', borderRadius: 10 }}>COVER</div>
                      )}
        ...
        {step === 5 && (
          <div>
            <div style={sectionStyle}>
              ...
              <div style={{ display: 'grid', gap: 14 }}>
                {[
                  ...
                  ['Price', `${form.currency} ${parseFloat(form.price || '0').toLocaleString()}`],
                  ['Location', [form.specific_location, form.subcity, form.city, form.region].filter(Boolean).join(', ')],
            ...
            <div style={{ background: 'white', borderRadius: 16, border: '2px solid #006AFF', padding: '24px 28px', marginBottom: 20 }}>
              ...
              <div style={{ fontSize: 13, color: '#6b7280', marginBottom: 16 }}>One-time payment to publish your listing. After review and payment confirmation, your listing will go live.</div>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
                  <div style={{ fontSize: 32, fontWeight: 900, color: '#006AFF' }}>ETB 500</div>
                  <div style={{ fontSize: 12, color: '#9ca3af' }}>One-time listing fee</div>
                </div>
            ...
            <div style={{ background: '#fffbeb', borderRadius: 12, border: '1px solid #fde68a', padding: '16px 20px', marginBottom: 20 }}>
              ...
              <div style={{ fontSize: 13, color: '#78350f', lineHeight: 1.7 }}>
                1. Submit listing → redirected to pay ETB 500<br />
                2. Payment confirmed → listing sent for review<br />
                3. Admin reviews within 24 hours<br />
                4. Listing goes LIVE on ጎጆ ✅
              </div>
            </div>
