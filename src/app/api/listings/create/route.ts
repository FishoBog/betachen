...
export async function POST(req: NextRequest) {
  try {
    ...
    // --- OPTIONAL server-side verification gate -----------------------------
    // Today's flow verifies the email via /api/verify/check at submit time on
    // the client. If you have a table that records verified emails (e.g.
    // "email_verifications" with a verified=true row), uncomment and adapt the
    // block below to hard-block unverified emails here. Until then this route
    // is no less safe than the previous client insert, which had no check.
    //
    // const sb = admin();
