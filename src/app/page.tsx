...
import { Search, MapPin, Home, Building2, BedDouble, Bath, Maximize2, Heart, ArrowRight, TrendingUp, Shield, Clock } from 'lucide-react';
...
const TYPE_LABELS: Record<string, { label: string; color: string; bg: string }> = {
  sale: { label: 'For Sale', color: '#1a3a5c', bg: '#e8f0f8' },
    ...
    <div style={{ minHeight: '100vh', background: '#ffffff' }}>
      ...
      <div style={{
        ...
        <div style={{ position: 'relative', maxWidth: 720, margin: '0 auto' }}>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: 'rgba(232,67,26,0.2)', border: '1px solid rgba(232,67,26,0.4)', borderRadius: 20, padding: '6px 16px', marginBottom: 24 }}>
            <MapPin size={14} color="#E8431A" />
            <span style={{ color: '#fca89d', fontSize: 13, fontWeight: 500 }}>Addis Ababa, Ethiopia</span>
          </div>
          
          <h1 style={{ fontSize: 52, fontWeight: 900, color: 'white', lineHeight: 1.1, marginBottom: 16, letterSpacing: '-1.5px' }}>
            Find Your Home<br />
            <span style={{ color: '#E8431A' }}>in Addis Ababa</span>
          </h1>
          <p style={{ color: 'rgba(255,255,255,0.65)', fontSize: 18, marginBottom: 40, lineHeight: 1.6 }}>
            Search thousands of properties for sale, rent, and short stay across all subcities
          </p>

          {/* Search box */}
          <div style={{ background: 'white', borderRadius: 16, padding: 8, display: 'flex', gap: 8, maxWidth: 600, margin: '0 auto', boxShadow: '0 20px 60px rgba(0,0,0,0.3)' }}>
            ...
            <button style={{ padding: '12px 28px', background: '#E8431A', color: 'white', border: 'none', borderRadius: 10, fontWeight: 700, fontSize: 15, cursor: 'pointer', whiteSpace: 'nowrap' }}>
              Search
            </button>
          </div>
          ...
          <div style={{ display: 'flex', justifyContent: 'center', gap: 40, marginTop: 40 }}>
            {[['1,200+', 'Properties'], ['50+', 'Subcities'], ['500+', 'Owners']].map(([num, label]) => (
        ...
        <div style={{ maxWidth: 1280, margin: '0 auto', display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
          {[['all', 'All Properties'], ['sale', 'For Sale'], ['long_rent', 'For Rent'], ['short_rent', 'Short Stay']].map(([val, label]) => (
            <button key={val} onClick={() => setTypeFilter(val)} style={{
              ...
              border: `2px solid ${typeFilter === val ? '#1a3a5c' : '#e5e7eb'}`,
              background: typeFilter === val ? '#1a3a5c' : 'white',
              ...
              return (
                <div key={p.id} style={{ background: 'white', borderRadius: 16, overflow: 'hidden', border: '1px solid #e5e7eb', transition: 'all 0.2s', cursor: 'pointer', boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}
                  ...
                  <Link href={`/property/${p.id}`} style={{ textDecoration: 'none' }}>
                    <div style={{ height: 200, background: 'linear-gradient(135deg, #e8f0f8, #dde8f0)', display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative' }}>
                      {p.images?.[0] ? (
                        <img src={p.images[0]} alt={p.title} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                      ) : (
                        <div style={{ textAlign: 'center', color: '#93c5fd' }}>
                          <Home size={48} strokeWidth={1} />
                          <div style={{ fontSize: 12, marginTop: 8, color: '#9ca3af' }}>No photo</div>
                        </div>
                      )}
                      {/* Badge */}
                  ...
                  <Link href={`/property/${p.id}`} style={{ textDecoration: 'none', color: 'inherit', display: 'block', padding: '18px 20px 20px' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 4, color: '#E8431A', fontSize: 13, marginBottom: 14 }}>
                      <MapPin size={13} />
                      {p.location || p.subcity || 'Addis Ababa'}
                    </div>
        ...
        <div style={{ maxWidth: 1280, margin: '0 auto', textAlign: 'center' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: 28 }}>
            {[
              ...
              { icon: TrendingUp, title: 'Market Insights', desc: 'Track prices and trends across all subcities' },
              { icon: Clock, title: 'Fast & Easy', desc: 'Post your property in under 5 minutes' },
              { icon: MapPin, title: 'Local Experts', desc: 'Deep knowledge of Addis Ababa neighborhoods' },
      ...
      {/* CTA */}
      <div style={{ background: '#1a3a5c', padding: '64px 24px', textAlign: 'center' }}>
        <h2 style={{ fontSize: 32, fontWeight: 800, color: 'white', marginBottom: 12 }}>Ready to list your property?</h2>
        <p style={{ color: 'rgba(255,255,255,0.65)', fontSize: 16, marginBottom: 32 }}>Join thousands of owners on ጎጆ</p>
