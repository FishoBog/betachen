...
export const revalidate = 3600; // cache 1 hour to stay under proxy limits
...
// Try multiple proxies so a single outage/cap doesn't break the feed
...
function hostName(feedUrl: string): string {
  try { return new URL(feedUrl).hostname.replace('www.', '').replace('.com', '').replace('.news', '').replace('.co', ''); }
  catch { return 'News'; }
}
...
export async function GET(req: NextRequest) {
  ...
  // Keyword filter, but if too few match keep everything so the tab isn't empty
  let articles = all;
