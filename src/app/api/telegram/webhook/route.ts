...
// Marks a Betachen listing-form session as verified when a user opens the bot
// via the deep link t.me/BetachenBot?start=<token> and presses Start. Returns
// true if the token was found and verified, so the caller can stop processing.
async function tryVerifyToken(token: string): Promise<boolean> {
  try {
...
export async function POST(req: NextRequest) {
  try {
    ...
    // ── BETACHEN VERIFICATION DEEP LINK ──
    // A deep link from the listing form arrives as "/start <token>". We intercept
    // it BEFORE the normal /start handler: verify the token, confirm to the user,
    // and stop. Plain "/start" (no token) falls through to the normal welcome.
