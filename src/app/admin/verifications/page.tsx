...
export default async function AdminVerificationsPage() {
  ...
  return (
    <div>
      <h2 className="text-2xl font-bold text-gray-900 mb-6">Owner Badge Verifications</h2>
                ...
                {req.status === "pending" && (
                  <div className="flex gap-2 flex-shrink-0">
                    <form action="/api/admin/verify" method="POST">
                      ...
                      <button name="action" value="approve" className="px-3 py-1.5 text-sm bg-green-600 text-white rounded-lg hover:bg-green-700 font-medium">
                        ✓ Approve
                      </button>
                    </form>
                    <form action="/api/admin/verify" method="POST">
                      <input type="hidden" name="requestId" value={req.id} />
                      <input type="hidden" name="userId" value={req.user_id} />
                      <button name="action" value="reject" className="px-3 py-1.5 text-sm bg-red-100 text-red-700 rounded-lg hover:bg-red-200 font-medium">
                        ✗ Reject
                      </button>
                    </form>
                  </div>
